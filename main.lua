...
local selectedProfileImage = "" -- 입력 코드에 따라 결정될 이미지 변수

-- 신규 인증 코드 및 이미지 ID 데이터베이스
local KeyDatabase = {
...
local function ShowBanScreen()
    local bannedGui = CreateBaseGui("ECA_BanSystem")
    bannedGui.IgnoreGuiInset = true; bannedGui.DisplayOrder = 100000
    local bg = Instance.new("Frame", bannedGui); bg.Size = UDim2.new(1, 0, 1, 0); bg.BackgroundColor3 = Color3.fromRGB(15, 0, 0)
    local banImg = Instance.new("ImageLabel", bg); banImg.Size = UDim2.new(0, 250, 0, 250); banImg.Position = UDim2.new(0.5, -125, 0.4, -125); banImg.Image = BAN_IMAGE; banImg.BackgroundTransparency = 1; banImg.ImageColor3 = Color3.fromRGB(255, 0, 0)
    local t = Instance.new("TextLabel", bg); t.Size = UDim2.new(1, 0, 0, 50); t.Position = UDim2.new(0, 0, 0.7, 0); t.Text = "ACCESS DENIED"; t.TextColor3 = Color3.fromRGB(255, 0, 0); t.TextSize = 40; t.Font = Enum.Font.SourceSansBold; t.BackgroundTransparency = 1
end
...
local function LoadActualMenu()
    ...
    local function ShowTab(tabName)
        ...
        elseif tabName == "Profile" then
            -- [동적 이미지 적용]
            local pImg = Instance.new("ImageLabel", contentFrame); pImg.Size = UDim2.new(0, 80, 0, 80); pImg.Position = UDim2.new(0.5, -40, 0, 0); pImg.Image = selectedProfileImage; Instance.new("UICorner", pImg).CornerRadius = UDim.new(1,0); pImg.BackgroundTransparency = 1
            local pName = Instance.new("TextLabel", contentFrame); pName.Size = UDim2.new(1, 0, 0, 30); pName.Position = UDim2.new(0, 0, 0, 90); pName.Text = "User: "..lp.Name; pName.TextColor3 = Color3.fromRGB(255,255,255); pName.BackgroundTransparency = 1
...
local function startLoading()
    ...
    task.spawn(function()
        ...
        -- 인증 이미지 추가 (AUTH_IMAGE)
        local midImg = Instance.new("ImageLabel", frame); midImg.Size = UDim2.new(0, 300, 0, 200); midImg.Position = UDim2.new(0.5, -150, 0.15, 0); midImg.Image = AUTH_IMAGE; midImg.BackgroundTransparency = 1
        ...
        local input = Instance.new("TextBox", frame); input.Size = UDim2.new(0.7, 0, 0, 40); input.Position = UDim2.new(0.15, 0, 0.65, 0); input.PlaceholderText = "인증 코드 입력..."; input.BackgroundColor3 = Color3.fromRGB(240, 240, 240); input.Text = ""
